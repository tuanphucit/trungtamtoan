...
class SystemPage extends CgsModules{
	...
	function execute(){		
		...
		Page::reg('js', Page::pathMod().'SystemPage.js');
		...
		$data = $dbObj->select('page','id,page_name,brief,layout,publish_flg,master_id,portal_id',$wh,'portal_id ASC, id DESC',null,'id');
		$this->block('BlockList');//print_r($data_portal);
		$stt = 0;
		foreach ($data as $row){
			$this->assign('stt', ++$stt);
			$this->assign('id', $row['id']);
			$this->assign('portal_name', isset($data_portal[$row['portal_id']]) ? $data_portal[$row['portal_id']]['portal_name'] : '[FALSE]');
