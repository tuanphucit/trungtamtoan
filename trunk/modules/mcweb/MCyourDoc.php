...
class MCyourDoc extends CgsModules{
	...
	function execute(){			
		...
		$total = $dbObj->count("document","`author` = '{$username}' AND `status` = 1 ","*");
		$allDoc = $dbObj->select('document', '*', "`author` = '{$username}' AND `status` = 1 ", 'updatetime DESC', $fistID . ','.$perPage);
		$paging = ClassPaging::paging($total, $perPage, array('page_no'=>$pageNo), 5,'page_no', 'active', '','');
		$stt = $fistID+1;		
		$htmlContent = "";
		if($allDoc){
