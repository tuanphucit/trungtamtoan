...
class EditGiasu extends CgsModules {
	...
	function execute() {
		...
		$form = new CgsFormsView('manager' . DS . 'MCwebTeacher.xml');
		...
		$this->assign('validate', $view->getHtml('validate1'));
		$this->assign('permission', $view->getHtml('permission'));
		...
		if ($submit) {
			...
			$permission = Page :: getRequest('permission', 'str', '', 'POST');
			$validate = Page :: getRequest('validate1', 'str', '', 'POST');
			...
			$submit = null;
			Page::goLink(null,'teacher','manager');
		}
		$html = $this->output();
		return $html;
	}
