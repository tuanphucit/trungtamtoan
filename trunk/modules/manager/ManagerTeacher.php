...
class ManagerTeacher extends CgsModules{
	...
	function execute(){		
		...
		Page::header('Danh sách gia sư');
		$pageNo = Page :: getRequest('page_no', int, 1, 'GET');
		$perPage = 20;
		$fistID = ($pageNo-1)*$perPage;
		$dbObj = new DefaultGiasuPeer();
		$total = $dbObj->count("giasu","","*");
		$allGiasu = $dbObj->select('giasu', 'giasu_id, username, updatetime, mobile, status, validate, userupdate', "", 'updatetime DESC', $fistID . ','.$perPage);
		...
		foreach ($allGiasu as $row){
			...
			$this->assign('detail_link', Page::link(array('id'=>$row['giasu_id']),'EditGiasu','manager'));
