...
class DefaultClassBase extends CgsDatabase {
	protected $__conn = 'default';
	protected $__tbl = 'class';
	protected $__prefix = '';
	public $field = '*';
	...
	function getRow($id=0,$field='*'){
		settype($id, 'int');
		return $this->selectOne($this->getTbl(), $field, "`class_id`={$id}");
	}
	
	function updateId($id=0,$data=array()){
		settype($id, 'int');
		return $this->update($this->getTbl(), $data, "`class_id`={$id}");
	}
